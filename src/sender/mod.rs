...
use crate::{DataTransmitted, DataPacket, PartitionDetails, Ticket, statistics::StatisticData, TransformStrategy};
...
where
    T: TransformStrategy {
        ...
        #[builder(private, default = "Arc::new(DashMap::with_capacity_and_hasher(2, BuildNoHashHasher::default()))")]
...
impl<T> KafkaPacketSender<T> where
    ...
    pub fn send_to_kafka(
        ...
        partition_detail: PartitionDetails) {
            ...
            spawn(async move {
                ...
                unsafe {
                    let payload = transform.transform(&addr, payload.get_unchecked(..len), &partition_detail.0);
